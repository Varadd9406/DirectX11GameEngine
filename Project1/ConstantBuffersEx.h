...
public:
	void Update(Graphics& gfx, const Dcb::Buffer& buf)
	{
		...
		DX_EXCEPT_THROW(GetContext(gfx)->Map(
...
protected:
		...
		slot(slot)
	{
		...
		if (pBuf != nullptr)
		{
			...
			DX_EXCEPT_THROW(GetDevice(gfx)->CreateBuffer(&cbd, &csd, &pConstantBuffer));
		}
		else
		{
			DX_EXCEPT_THROW(GetDevice(gfx)->CreateBuffer(&cbd, nullptr, &pConstantBuffer));
		}
	}
private:
