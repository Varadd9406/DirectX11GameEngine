...
{
public:
	static IndexedTriangleList MakeTesselated(dymvtx::VertexLayout layout, int latDiv, int longDiv)
	{
		...
		dymvtx::VertexBuffer vb{ std::move(layout) };
	...
	static IndexedTriangleList Make(std::optional<dymvtx::VertexLayout> layout = std::nullopt)
	{
		using Element = dymvtx::VertexLayout::ElementType;
		if (!layout)
		{
			layout = dymvtx::VertexLayout{}.Append(Element::Position3D);
		}
